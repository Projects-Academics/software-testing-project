...
url = "https://api.themoviedb.org/3/tv/1/season/1/episode/1/credits?language=pt-BR"
...
def test_get_credits_sucesso():
    """Testar se o GET request foi bem sucedido"""
    response = requests.get(url, headers=headers)
    
    assert response.status_code == 200
    assert "cast" in response.text or "crew" in response.text
    print(f"Status Code: {response.status_code}")
    print(f"Response: {response.text}")

def test_get_credits_sem_autorizacao():
    """Testar se o GET request falha sem autorização"""
